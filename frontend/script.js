...
const UI = {
  ...
  updateThemeColors() { /* ...mesmo que antes... */ },
  updateThemeToggleButton() { /* ...mesmo que antes... */ },
  renderHistory() { /* ...mesmo que antes... */ },
  renderFavorites() { /* ...mesmo que antes... */ },
  toggleThemeColors() { /* ...mesmo que antes... */ },
  applySavedTheme() { /* ...mesmo que antes... */ }
};
...
const App = {
  ...
  async fetchByCoords(lat, lon) {
    ...
    try {
      ...
      const fallbackCity = "São Miguel do Oeste";
      this.handleCitySelect(fallbackCity);
    } finally {
      dom.weatherDiv.classList.remove("loading");
    }
  },
  addFavorite(city) { /* ...mesmo que antes... */ },
  async removeFavorite(city) { /* ...mesmo que antes... */ },
  updateButtonsState() { /* ...mesmo que antes... */ },
  init() {
    UI.applySavedTheme();
    UI.renderHistory();
    UI.renderFavorites();
    ...
    dom.favBtn.addEventListener("click", () => { const city = normalizeCityInput(dom.cityInput.value); if (!city) return; this.addFavorite(city); });
    dom.themeToggle.addEventListener("click", () => UI.toggleThemeColors());
    const fallbackCity = "São Miguel do Oeste";
    const lastCity = Storage.getLastCity();
    if (lastCity) this.handleCitySelect(lastCity);
    else if (navigator.geolocation) navigator.geolocation.getCurrentPosition(pos => this.fetchByCoords(pos.coords.latitude, pos.coords.longitude), () => this.handleCitySelect(fallbackCity));
    else this.handleCitySelect(fallbackCity);
  }
};
